...
--
-- Insere uma variavel, caso não exista, ou atualiza caso exista.
-- Equivalente ao REPLACE do MySQL
--
CREATE OR REPLACE FUNCTION sp_salvar_variavel_sistema(p_chave character varying, p_valor character varying)
  RETURNS void AS
$BODY$
BEGIN
    IF EXISTS( SELECT 1 FROM variaveis_sistema WHERE chave = p_chave ) THEN
        UPDATE variaveis_sistema
        SET valor = p_valor
        WHERE chave = p_chave;
    ELSE
        INSERT INTO variaveis_sistema VALUES( p_chave,  p_valor );
    END IF;
END;
$BODY$
  LANGUAGE plpgsql;
...
INSERT INTO usuarios (id_usu, login_usu, nm_usu, ult_nm_usu, senha_usu, ult_acesso, stat_usu) VALUES (1, '%login_usu%', '%nm_usu%', '%ult_nm_usu%', md5('%senha_usu%'), NULL, 1);
