				...
				for($y=$this->a+($this->larg/2),$valuelen=2,$i=0;$i<count($this->value[$t]);$i++,$y+=$this->ld,$valuelen=2)
				{
					...
					if(isset($this->ExtLeg)){
						...
						switch($this->ExtLeg){
                            ...
                            case 3: $strExtVal = call_user_func($this->funcVly, $this->value[$t][$i]); break;
						}$this->ExtLeg;
					}
	...
	function gradAxis($sy=NULL,$sx=NULL)
	{
		...
		for($x=$zero ; $x <= $this->w-$this->d+1 ; $n+=$this->dvx,$x+=$this->dvx*$this->mul)
		{
            if (!empty($this->funcVly)) {
                $str = call_user_func($this->funcVly, $n);
            }
            else {
                $str = $n;
            }
			...
			imagefttext($this->im,$this->font_small,0,$x1,$y1,$this->font_color,$this->font, $str);
		}
		if($sy)
