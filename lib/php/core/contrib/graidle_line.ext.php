				...
				for($x=0, $x1=(($this->larg+$this->disbar)/2)*$AA ,$i=0,$i1=1;$i1<count($this->value[$t]);$i++,$i1++,$x1=$x2)
				{
					if($x==0)
					{
                        ...
                        if (!empty($this->funcVly)) {
                            $str = call_user_func($this->funcVly, $this->value[$t][$i]);
                        }
                        else {
                            $str = $this->value[$t][$i];
                        }
						...
						if($this->mnvs<=0)
						{
                            ...
                            if ($this->legend > 0) {
                                if ($i < 2) {
                                    imagefttext($img_tmp, $this->font_small*$AA,0,$x1-(($this->font_small)*graidle::stringlen($str)*0.5),$y1-($this->font_small/2),$cc,$this->font, $str);
                                }
                                else if ($i == 2) {
                                    imagefttext($img_tmp, $this->font_small*$AA,0,$x1-(($this->font_small)*graidle::stringlen($str)*0.5),$y1-($this->font_small/2),$cc,$this->font, $str);
                                    imagefttext($img_tmp, $this->font_small*$AA,0,$x2-(($this->font_small)*graidle::stringlen($str)*0.5),$y2-($this->font_small/2),$cc,$this->font,$str);
                                }
                                else {
                                    imagefttext($img_tmp, $this->font_small*$AA,0,$x2-(($this->font_small)*graidle::stringlen($str)*0.5),$y2-($this->font_small/2),$cc,$this->font,$str);
                                }
