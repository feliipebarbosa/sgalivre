...
try {
	...
	$rel->setTitulo("Estatísticas por Macrosserviços");
    ...
    if ($est_agregado) {
        $tabela = Estatistica::get_estatisticas_servicos_mestres("Estatísticas por Macrosserviços - Agregado", $ids_uni, $dt_min, $dt_max);
    ...
    if ($est_unidades) {
        foreach ($tmp as $u) {
            $tabela = Estatistica::get_estatisticas_servicos_mestres("Estatísticas por Macrosserviços - ".$u->get_nome(), array($u->get_id()), $dt_min, $dt_max);
            $rel->addComponente($tabela);
