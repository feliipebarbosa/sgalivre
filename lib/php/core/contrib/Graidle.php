...
class Graidle
{
	...
	function Graidle($title=NULL,$mass=NULL,$mnvs=NULL){
		...
		$this->mnvs=$mnvs;
		$this->fontMono=FALSE;
        $fontPath = dirname($_SERVER['SCRIPT_FILENAME'])."/lib/php/core/contrib/";
		graidle::setFont($fontPath."/Vera.ttf");
		graidle::setFontBD($fontPath."/VeraBd.ttf");
		graidle::setFontLegend($fontPath."/Vera.ttf");
		$this->width=NULL;
		$this->height=NULL;
		$this->xAxis=NULL;
		$this->yAxis=NULL;
	...
	function create(){
		if ($this->cnt == 0) {
            fatal_error(iconv("UTF-8", "ISO-8859-1//TRANSLIT", $this->title."\nNenhum atendimento no período"));
        }
        if(in_array("b",$this->type)||in_array("l",$this->type))
		{
			...
			else
			{
                ...
                $this->ld = ($this->w - $this->s - $this->d)/$this->cnt;
                $this->larg = $this->ld/2;
				$this->disbar=$this->larg;
		...
		else if(in_array("hb",$this->type))
		{
			...
			if(isset($this->yAxis))	$this->s+=2*$this->fontsmall;
	...
	function gradAxis($sy=NULL,$sx=NULL){
		...
		while($zero-$n >= $this->a)
		{
			...
			$v=round($n/$this->mul);
            if (!empty($this->funcVly)) {
                $str = call_user_func($this->funcVly, $v);
            }
            else {
                $str = $v;
            }
			imagefttext($this->im,$this->font_small,0,$this->s-(($this->font_small)*graidle::stringlen($str)),($zero-$n)+($this->font_small/2),$this->font_color,$this->font,$str);
			$n+=round($this->dvx*$this->mul);
		}
		#Under zero
	...
	function setFont($font,$size=8){
		...
		graidle::setFontBigSize($size*1.75);
	}
    ...
    function setYValueFunc($funcVly){
		$this->funcVly=$funcVly;
	}
	...
	function setExtLegend($type=0){
		switch($type){
            ...
            case 3 :	$this->ExtLeg=3;break;	#Processed by display function
