...
try {
	...
	$rel->setTitulo("Variação dos Tempos Médios");
    ...
    if ($gfc_agregado) {
        $graidle = Graficos::get_linha_tempos_medios_por_periodo("Tempos Médios - Variação Mensal - Agregado", $ids_uni, $dt_min, $dt_max);
    ...
    if ($gfc_unidades) {
        foreach ($tmp as $u) {
            $graidle = Graficos::get_linha_tempos_medios_por_periodo("Tempos Médios - Variação Mensal - ".$u->get_nome(), array($u->get_id()), $dt_min, $dt_max);
