...
try {
	...
	$rel->setTitulo("Estatísticas por Serviços Codificados");
    ...
    if ($est_agregado) {
        $tabela = Estatistica::get_estatisticas_servicos_codificados("Estatísticas por Serviços Codificados - Agregado", $ids_uni, $dt_min, $dt_max);
    ...
    if ($est_unidades) {
        foreach ($tmp as $u) {
            $tabela = Estatistica::get_estatisticas_servicos_codificados("Estatísticas por Serviços Codificados - ".$u->get_nome(), array($u->get_id()), $dt_min, $dt_max);
            $rel->addComponente($tabela);
    ...
    $tabela = Estatistica::get_legendas_tempos_medios();
    $rel->addComponente($tabela);
    $rel->addComponente(Separador::getInstance());
