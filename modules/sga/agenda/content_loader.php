...
try {
	if (Session::getInstance()->exists('AGENDA_PAGINA')) {
       $link = "modules/sga/agenda/" . DB::getInstance()->get_menu_link(Session::getInstance()->get('AGENDA_PAGINA'));
        if (file_exists($link)) {
        	SGA::_include("$link");        	
        } else {
      	  	Session::getInstance()->del('AGENDA_PAGINA');
